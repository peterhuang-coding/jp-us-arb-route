...
import {initMarket, dealFields, readDeal} from './market-board.js?v=20260914-1';
...
let opener = null, editorSession = 0, draftEvidenceId = '';
const descriptions = {
...
const evidenceKindName = kind => ({ask:'挂牌参考',sold:'历史成交',offer:'有效报价',order:'自己的订单'}[kind] || '待核');
const cnLocal = value => value ? new Date(new Date(value).getTime() + 8 * 3600000).toISOString().slice(0,16) : '';
function evidenceHistory(item) {
  const records = item.quote?.evidence_records || [];
  if (!records.length) return '<p class="evidence-empty">尚无可追溯销售依据。旧手填卖价不会直接放行采购。</p>';
  return `<div class="evidence-history">${[...records].reverse().map(e => `<article><div><strong>${esc(evidenceKindName(e.kind))} · ${esc(cny(e.amount_basis === 'gross' ? e.amount_cny - (e.fees_cny || 0) : e.amount_cny))} 净收</strong><small>${esc(e.channel)} · ${esc(e.observed_at ? new Date(e.observed_at).toLocaleString('zh-CN',{timeZone:'Asia/Shanghai',hour12:false}) : '时间待核')}</small></div><p>${esc(e.code || '货号待核')} · ${esc(e.color || '版本待核')} · ${esc(e.size || '规格待核')}<br>${esc(e.source_ref || '来源待核')}</p>${e.source_url ? `<a href="${esc(e.source_url)}" target="_blank" rel="noopener noreferrer">查看依据 ↗</a>` : ''}</article>`).join('')}</div>`;
}
function openEditor(item = null) {
  editorSession += 1;
  draftEvidenceId = crypto.randomUUID ? crypto.randomUUID() : `evidence-${Date.now()}-${Math.random()}`;
  ...
  $('#editor-fields').innerHTML = `<section class="form-section"><h3>商品与采购点</h3><div class="form-grid">${field('name','商品名',i.name)}${select('pool','观察类型',i.pool,[['stable','常卖款候选'],['hot','热点款候选']])}${field('code','准确货号',i.code)}${field('color','配色 / 版本',i.color)}${field('size','准确尺码 / 规格',i.size)}${select('channel','国内销售渠道',i.channel,[['得物','得物'],['闲鱼','闲鱼']])}${field('source_name','采购网站 / 门店名',i.source_name)}${field('source_url','具体商品页链接',i.source_url,'url')}${field('address','东京取货点 / 收货安排（勿填买家隐私）',i.address,'text',true)}</div><p>${esc(i.reason || '从准确商品开始；抽签、补货或联名信息还需逐项核实。')}</p></section>
    ${dealFields(i)}${visitFields(i)}<section class="form-section"><h3>每件采购成本</h3><div class="form-grid">${field('buy_jpy','含税采购价 · JPY（竞拍含另收费用，自动计算）',q.buy_jpy,'number')}${field('fx','1 JPY 兑换 CNY（含换汇成本）',q.fx,'number')}${field('extra_cny','其余每件成本 · CNY',q.extra_cny,'number')}${field('target_profit','每件目标利润 · CNY（初始建议 100）',q.target_profit,'number')}${field('units','计划件数',q.units,'number')}${field('checked_at','实际采购价 / 库存核验日期',q.checked_at,'date')}</div><p>其余成本包含商业转售税费、日本与国内运费、包材、额外行李及分摊费用。未知项留空，确实为零才填 0。</p></section>
    <section class="form-section"><h3>添加一条国内销售依据</h3><div class="form-grid">${select('evidence_kind','依据类型','unknown',[['unknown','本次不添加'],['ask','他人挂牌 / 平台展示价'],['sold','可核对的同规格成交记录'],['offer','当前真实买家询价 / 有效报价'],['order','自己的真实订单']])}${select('evidence_basis','金额口径','net',[['net','卖家预计净收入（已扣销售费用）'],['gross','成交 / 报价毛额（另填销售费用）']])}${field('evidence_amount','观察金额 · CNY',null,'number')}${field('evidence_fees','销售侧扣款 · CNY（净额口径留空）',null,'number')}${field('evidence_observed','准确观察时刻 · 中国时间',cnLocal(new Date()),'datetime-local')}${field('evidence_url','原始页面 / 凭据链接',null,'url',true)}<label class="field wide">来源说明（必填，不含姓名、电话和地址）<textarea name="evidence_source" maxlength="500" placeholder="例如：得物卖家端，同货号同尺码最近成交；或闲鱼真实买家有效报价"></textarea></label><label class="field wide">补充说明（可选）<textarea name="evidence_note" maxlength="1000" placeholder="记录成色、包装、费用范围或订单状态"></textarea></label></div><p>保存时会把当前货号、配色和规格快照写入依据。挂牌价只作参考；成交、有效报价或自己的订单在近 72 小时内才参与采购上限。新增观察会保留历史，不覆盖旧依据。</p>${evidenceHistory(i)}</section>
...
function readForm({strictEvidence = false} = {}) {
  ...
  for(const key of ['buy_jpy','fx','extra_cny','target_profit','units']) item.quote[key] = data.get(key) === '' ? null : Number(data.get(key));
  item.quote.checked_at = data.get('checked_at') || null;
  const evidenceKind = String(data.get('evidence_kind') || 'unknown');
  if(evidenceKind !== 'unknown'){
    const sourceRef=String(data.get('evidence_source')||'').trim(),amount=Number(data.get('evidence_amount'));
    const observed=String(data.get('evidence_observed')||'');
    const basis=String(data.get('evidence_basis')||'net'),feesRaw=data.get('evidence_fees');
    const fees=feesRaw===''?null:Number(feesRaw),sourceUrl=String(data.get('evidence_url')||'').trim();
    if(strictEvidence){
      const missing=[];
      if(!sourceRef)missing.push('来源说明');
      if(!Number.isFinite(amount)||amount<=0)missing.push('有效观察金额');
      if(!observed)missing.push('准确观察时刻');
      if(!item.code||!item.color||!item.size)missing.push('商品货号、配色和规格');
      if(basis==='gross'&&(!Number.isFinite(fees)||fees<0))missing.push('销售侧扣款');
      if(basis==='gross'&&Number.isFinite(fees)&&fees>=amount)throw new Error('销售侧扣款必须小于观察毛额');
      if(sourceUrl&&!/^https?:\/\/[^\s]+$/i.test(sourceUrl))throw new Error('销售依据链接须以 http:// 或 https:// 开头');
      if(missing.length)throw new Error('销售依据还缺：'+missing.join('、'));
    }
    if(sourceRef && Number.isFinite(amount) && amount>0 && observed){
      const record={id:draftEvidenceId,kind:evidenceKind,channel:item.channel,amount_cny:amount,amount_basis:basis,fees_cny:fees,observed_at:observed+':00+08:00',source_ref:sourceRef,source_url:sourceUrl,note:String(data.get('evidence_note')||'').trim(),code:item.code,color:item.color,size:item.size};
      item.quote.evidence_records=[...(item.quote.evidence_records||[]).filter(e=>e.id!==draftEvidenceId),record];
    }
  }
...
function liveDiagnosis() {
  ...
  $('#live-diagnosis').innerHTML = `<div class="calc-grid"><div><small>保守净收入 / 件</small><strong>${esc(cny(d.exitEvidence?.net_cny??null))}</strong></div><div><small>预计净赚 / 件</small><strong>${esc(cny(d.profit))}</strong></div><div><small>${auction?'最高含税出价':'最高采购价'} · JPY</small><strong>${esc(jpy(auction?d.maxBidJPY:d.maxBuyJPY))}</strong></div></div><p>${d.exitEvidence ? `依据：${esc(evidenceKindName(d.exitEvidence.kind))} · ${esc(d.exitEvidence.source_ref)} · ${esc(new Date(d.exitEvidence.observed_at).toLocaleString('zh-CN',{timeZone:'Asia/Shanghai',hour12:false}))}` : '添加有效销售依据后，系统才计算采购上限。'}${d.stressProfit === null ? '' : '<br>净收入下降 10% 后，预计每件利润 ' + esc(cny(d.stressProfit)) + '。'}</p>${d.ready ? '<p><strong>核价条件齐全。保存后可加入采购单，合计仍受预算限制。</strong></p>' : `<ul>${d.blockers.map(b=>`<li>${esc(b)}</li>`).join('')}</ul>`}`;
}
...
async function save(event) {
  ...
  let item;
  try { item=readForm({strictEvidence:true}); }
  catch(e) { $('#form-error').textContent='保存失败：'+e.message+'。'; return; }
...
function exportCSV() {
  ...
  const rows=[['东京采购清单（估算，非已下单）','2026-10-02 至 2026-10-07'],['商品','货号','配色','尺码','采购点','具体商品页','东京取货安排','销售渠道','数量','采购价 JPY/件（竞拍按下一加价且含另收拍卖费）','最高买价 JPY/件（竞拍为含税出价，另收拍卖费另计）','全部成本 CNY/件','预计净利 CNY/件','销售依据类型','销售依据说明','销售依据链接','采购核验日期','销售依据时刻','收件节点']];
  for(const i of plan.selected){const q=purchaseQuote(i),d=diagnose(i),e=d.exitEvidence;rows.push([i.name,i.code,i.color,i.size,i.source_name,i.source_url,i.address,i.channel,q.units,q.buy_jpy,i.deal?.kind==='auction'?d.maxBidJPY:d.maxBuyJPY,d.cost,d.profit,e?.kind,e?.source_ref,e?.source_url,q.checked_at,e?.observed_at,'待真实订单生成；按订单寄平台指定点或买家']);}
