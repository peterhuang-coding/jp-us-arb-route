...
白天把一张卡改为“今晚执行”，或在当前 Codex 任务明确选择；夜间按 [执行规则](docs/day-night-workflow.md) 完成已选任务，再回写 Notion 和 GitHub。当前均待选，每天北京时间 01:00 的定时跟进已配置，尚未首次夜跑。调研完成后再决定开发；该安排由 Codex 执行，网页应用尚未接入 Notion 实时同步。

恢复入口：[版本化主线](.claude/autopilot/mainline.md)。转售建议先验证 R1 的销路和净价；HOKA 自用找货可单选 R2。

## 证据驱动买入决策卡（2026-09-14）

当前 MVP 聚焦“从线索到可执行买入”的转化，而不是全网热门推荐或自动抢购。每个准确 SKU 的决策卡需要回答：在哪里买、最高多少钱买、卖到哪个渠道、预计净赚多少、使用哪条销售依据、还缺哪些条件。

- 国内销售依据按记录保存，不再只手填一个卖价。每条记录包含渠道、金额口径、销售费用、带时区观察时刻、来源说明/链接和货号、配色、规格快照。
- 只有同渠道、同规格、近 72 小时的成交、有效报价或自己的订单参与采购判断；挂牌价只作参考。
- 多条有效依据取最低净收入，毛额先扣销售侧费用。观察池、今日买卖、日计划、工作流和 CSV 使用同一结果。
- 旧 `net_cny` 和自由文本依据字段继续兼容存储，但不能单独放行采购；修改规格或渠道后，历史依据保留但自动失效。
- 当前没有自动抓取得物/闲鱼行情、自动竞价或自动下单，也尚未用真实交易证明收益提升。
...
```bash
python -m pytest tests/test_sourcing.py tests/test_day_plans.py -q
node --test --test-concurrency=2 tests/sourcing-model.test.mjs tests/market-decisions.test.mjs tests/tokyo-planner.test.mjs
```
...
- 点击“核价与判断”录入实际含税买价、汇率、其余成本、结构化销售依据与取货条件；未知成本不能当作零。
